...
#' @description Generates setup list of length 5 to store 
...
#' @param filename (required) The most recently saved .RData file 
#' produced by SMART. It will be found in the R_data folder.
#' @param setup (required) The setup variable produced during the 
#' [setup_pl()] function. 
#' @param output_folder (required) The output folder that will 
#' contain all files required for the applet. The user must create
#' this folder prior to running the function, since the function
#' will create subfolders within *output_folder*.
#' @return Returns *shiny_setup*, a list containing the following
#' information: .RData file name, setup variable, output folder,
...
shiny_setup <- function(filename, setup, output_folder){
  ...
  shiny_setup <- list(filename = filename, setup = setup, output_folder = output_folder, output_folders = output_folders)
  return(shiny_setup)
}
